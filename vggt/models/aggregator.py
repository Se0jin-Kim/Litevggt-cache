...
class Aggregator(nn.Module):
    ...
    def forward(self, images: torch.Tensor, use_adaptive_cache: bool = True) -> Tuple[List[torch.Tensor], int]:
        """
        Args:
    ...
    def _process_global_attention(self, tokens, B, S, P, C, global_idx, pos=None, info_map=None,
        ...
        if use_adaptive_cache:
            b_idx_cached = self.m_u[2] if self.m_u is not None else None
            recompute, reason = self.adaptive_cache.should_recompute(
        ...
        if use_adaptive_cache and cal_merge:
            b_idx_new = m_u[2]
            self.adaptive_cache.update_cache(global_idx, m_u, tokens, b_idx_new)
        elif use_adaptive_cache and not cal_merge:
            self.m_u = self.adaptive_cache.get_m_u(global_idx)
